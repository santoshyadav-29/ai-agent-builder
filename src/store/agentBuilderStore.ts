...
const normalizeSavedAgents = (
  savedAgents: Array<SavedAgent | PersistedSavedAgent>,
): SavedAgent[] => {
  ...
  persist(
    (set, get) => ({
      ...
      deleteSavedAgent: (idToRemove) => {
        set((state) => ({
          savedAgents: state.savedAgents.filter((agent) => agent.id !== idToRemove),
        }));
      },
    ...
    }),
    {
      ...
      merge: (persistedState, currentState) => {
        const incoming = persistedState as Partial<AgentBuilderState> | undefined;
        const persistedAgents = normalizeSavedAgents(
          ((incoming?.savedAgents as Array<SavedAgent | PersistedSavedAgent>) ?? []),
        );
